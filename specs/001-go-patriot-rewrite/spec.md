...
- **FR-023**: System MUST expose supply organizer with truck+warehouse+hazmat — `GET /admin/supplies` filterable `location=warehouse|truck:{tech_id}|hazmat` (`supplies{sku, name, qty, reorder_point, location, hazmat:bool, sds_url, osha_class}` + `supply_locations{location_type, tech_id?}`), check-in/out `POST /api/supplies/{id}/checkin`+`POST /api/supplies/{id}/checkout` with **photo `photo_url`**, `qty`, `tech_id`, `gps`, `timestamp`, `reason` → `supply_moves{who, what, qty, photo_url, gps, at}` OSHA ledger `audit_log action=supply.checkin|checkout`; full history `GET /admin/supplies/{id}/ledger`, low-stock + hazmat expiry alerts, link to `kanban_cards`/`appointments`/`tech/routes`, FieldRoutes sync if inventory API exists else native.
...
- **Supply**: `supplies{sku, name, qty, reorder_point, location=warehouse|truck:{tech_id}|hazmat, hazmat, sds_url, osha_class}`+`supply_locations`+`supply_moves{who, qty, photo_url, gps, at, reason}` OSHA ledger `supply.checkin|checkout` + `GET /admin/supplies/{id}/ledger` low-stock/hazmat alerts; FieldRoutes sync or native
